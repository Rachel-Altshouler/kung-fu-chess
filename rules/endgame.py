from domain.constants import BoardConstants, Colors
...
class EndgameRules:
    ...
    @staticmethod
    def evaluate(board):
        white_king = EndgameRules.find_king(board, Colors.WHITE)
        black_king = EndgameRules.find_king(board, Colors.BLACK)
        if white_king is None and black_king is not None:
            return "king_captured", Colors.BLACK
        if black_king is None and white_king is not None:
            return "king_captured", Colors.WHITE
        return None
