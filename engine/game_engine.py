...
from domain.constants import Colors, PieceTypes, BoardConstants
...
class GameEngine:
    def __init__(self, board, event_bus=None):
        ...
        self._game_end_published = False
    ...
    def notify_game_started(self):
        self._publish(Events.GAME_STARTED, {"clock": self.get_clock()})
    ...
    def get_move_log(self):
        return self._state.get_move_log().get_all()
    ...
    def _apply_jump_captures(self):
        ...
        for movement in captured:
            ...
            if capturer_color is not None:
                ...
                self._publish(
                    Events.PIECE_CAPTURED,
                    {
                        "capturing_color": capturer_color,
                        "captured_token": movement.piece_token,
                    },
                )
            ...
            if destination != movement.source:
                ...
                if (
                    ...
                    if (
                        ...
                        logged_capture = captured_token
                        self._publish_score_updated(mover.color, captured_token)
                        self._publish(
                            Events.PIECE_CAPTURED,
                            {
                                "capturing_color": mover.color,
                                "captured_token": captured_token,
                            },
                        )
                board.move_piece(movement.source, destination)
                self._promote_pawn_if_needed(board, destination)
                self._record_completed_move(movement, destination, logged_capture)
    ...
    def _record_completed_move(self, movement, destination, captured_token=None):
        board = self._state.get_board()
        grid = board.get_grid()
        record = MoveRecord(
            piece_token=movement.piece_token,
        ...
        self._state.get_move_log().add(record)
        self._publish(
            Events.MOVE_LOGGED,
            {"record": record},
        )

    def _publish_score_updated(self, capturing_color, captured_token):
        self._publish(
            Events.SCORE_UPDATED,
            {
                "white": self._state.get_white_score(),
                "black": self._state.get_black_score(),
                "capturing_color": capturing_color,
                "captured_token": captured_token,
            },
        )

    def _publish_game_ended_if_needed(self):
        if not self._state.is_game_over() or self._game_end_published:
            return
        self._game_end_published = True
        self._publish(
            Events.GAME_ENDED,
            {
                "winner": self._state.get_winner(),
                "is_stalemate": self._state.is_stalemate(),
            },
        )
    ...
    def _evaluate_endgame(self):
        ...
        self._publish_game_ended_if_needed()
    ...
    def _smooth_progress(t: float) -> float:
        """Ease-in-out so walking accelerates/decelerates naturally."""
    ...
    def _position_at_time(movement, clock):
        ...
        for index in range(len(path) - 1):
            ...
            if clock < time2:
                ...
                # Jumps stay linear; walks use smooth easing
                if not getattr(movement, "is_jump", False):
                    progress = GameEngine._smooth_progress(progress)
