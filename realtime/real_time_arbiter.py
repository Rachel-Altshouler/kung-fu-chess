from domain.constants import BoardConstants, PieceTypes
from domain.piece import Piece
from realtime.motion import Motion

#מה קורה ששני כלים מתנגשים
class RealTimeArbiter:
    @staticmethod
    #המטרה: לזהות מקרים שבהם פרש (או כלי קופץ אחר) "נוחת" על כלי שעדיין נמצא בתנועה, ולתפוס אותו "באוויר" — לפני שהוא בכלל מגיע ליעד שלו.
    ...
    @staticmethod
    #בהינתן תנועה שנתפסה, מוצאת מי תפס אותה-כדי לדעת למי לזקוף את האכילה
    ...
    @staticmethod
    #זו פונקציה שקובעת איפה כלי בתנועה "באמת" יסתיים, בהתחשב בכל שאר התנועות שקורות במקביל
...
#זה קובץ שפותר בעיה עדינה מאוד ב"שחמט בזמן אמת ללא תורות": מה קורה כששני שחקנים זזים באותו רגע ממש, והמסלולים שלהם נפגשים? הפתרון:
# לכל תנועה יש movement_id (כנראה עולה לפי סדר יצירה/זמן התחלה), וה**"כלל הבסיסי" הוא "מי שהתחיל לזוז קודם — מנצח את המרוץ"** למשבצת המשותפת.
# זה מונע מצבים לא הגיוניים כמו "שני כלים תופסים זה את זה בו-זמנית" או "כלי חוצה קיר בלתי-אפשרי".
