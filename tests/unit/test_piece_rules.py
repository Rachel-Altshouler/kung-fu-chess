...
class TestPieceRules(unittest.TestCase):
    ...
    def test_knight_jumps_over_blocker(self):
        ...
        self.assertTrue(
            PieceRules.is_piece_move_valid(blocked_grid, 'N', (0, 0), (1, 2), 'w')
        )
    ...
    def test_pawn_invalid_color(self):
        grid = [
            ['.', '.', '.'],
            ['.', 'wP', '.'],
            ['.', '.', '.']
        ]
        self.assertFalse(PieceRules.is_pawn_move(grid, (1, 1), (0, 1), 'x'))

    def test_pawn_invalid_lateral_move(self):
        grid = [
            ['.', '.', '.'],
            ['.', 'wP', '.'],
            ['.', '.', '.']
        ]
        self.assertFalse(PieceRules.is_pawn_move(grid, (1, 1), (1, 2), 'w'))
        self.assertFalse(PieceRules.is_pawn_move(grid, (1, 1), (0, 3), 'w'))

    def test_unknown_piece_type(self):
        self.assertFalse(
            PieceRules.is_piece_move_valid(self.grid, 'Z', (0, 0), (1, 1), 'w')
        )
